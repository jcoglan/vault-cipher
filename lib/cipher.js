...
Cipher.prototype.deriveKeys = function() {
  ...
  var encryptKey = key.slice(0, this._mode.keySize),
...
Cipher.prototype.decrypt = function(ciphertext, callback, context) {
  var keyPair    = this.deriveKeys(),
      ...
      message    = buffer.slice(0, Math.max(buffer.length - this._mac.outputSize, 0)),
      iv         = message.slice(0, Math.min(this._mode.blockSize, message.length)),
      payload    = message.slice(Math.min(this._mode.blockSize, message.length)),
      mac        = buffer.slice(Math.max(buffer.length - this._mac.outputSize, 0)),
