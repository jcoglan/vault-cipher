...
var UTF8 = {
  decode: function(string) {
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      if (c >= 0xd800 && c <= 0xdbff) {
        ...
        if (d < 0xdc00 || d > 0xdfff) throw new Error('Illegal low surrogate ' + d + ' at offset ' + i);
  ...
  encode: function(data) {
    ...
    while (offset < n) {
      b = [data.charCodeAt(offset), data.charCodeAt(offset + 1), data.charCodeAt(offset + 2), data.charCodeAt(offset + 3)];
