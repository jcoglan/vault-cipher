  ...
  encode: function(data) {
    ...
    while (offset < n) {
      ...
      if (c <= 127) {
        ...
        for (i = 0; i < 5; i++) binary += pad(data.charCodeAt(offset + i), 2, 8);
        match = binary.match(/^111110(.{2})10(.{6})10(.{6})10(.{6})10(.{6})$/);
        offset += 5;
      } else if (c <= 253) {
        for (i = 0; i < 6; i++) binary += pad(data.charCodeAt(offset + i), 2, 8);
      ...
      if (c <= 0xffff) {
        ...
        c -= 0x10000;
        codepoints.push(0xd800 + (Math.floor(c / 0x400) & 0x3ff), 0xdc00 + (c & 0x3ff));
      }
    }
