var crypto_shim = {
  createCipheriv: function(cipherMode, key, iv) {
    return new crypto_shim.Cipher(cipherMode, key, iv);
  },

  createDecipheriv: function(cipherMode, key, iv) {
    return new crypto_shim.Decipher(cipherMode, key, iv);
  },

  createHmac: function(hashMode, key) {
    return new crypto_shim.Hmac(hashMode, key);
  },
...
crypto_shim.Cipher.prototype.update = function(chunk, inputEncoding, outputEncoding) {
  if (typeof chunk === 'string') chunk = new Buffer(chunk, inputEncoding);
  this._text = this._text.concat(chunk);
  return '';
};

crypto_shim.Cipher.prototype.final = function(outputEncoding) {
  var message   = this._text.toString('utf8'),
...
crypto_shim.Hmac.prototype.update = function(chunk, inputEncoding, outputEncoding) {
  if (typeof chunk === 'string') chunk = new Buffer(chunk, inputEncoding);
  this._text = this._text.concat(chunk);
  return '';
};
