...
Cipher.prototype.encrypt = function(plaintext, callback, context) {
  this.deriveKeys(function(key1, key2) {
    var key = new Buffer(key1, 'utf8'),
        ...
        ciphertext = cipher.update(plaintext, 'utf8', this._format) + cipher.final(this._format);
    ...
    hmac.update(result.toString(this._format));
  ...
  this.deriveKeys(function(key1, key2) {
    try {
      var key       = new Buffer(key1, 'utf8'),
          ...
          plaintext = cipher.update(payload, this._format, 'utf8') + cipher.final('utf8');
    }
    catch (e) {
    ...
    var hmac = crypto.createHmac(this._mac, key2);
    hmac.update(message.toString(this._format));
