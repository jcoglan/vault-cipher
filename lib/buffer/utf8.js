'use strict';

module.exports = {
  decode(string) {
    let bytes = [],
        n     = string.length;

    for (let i = 0; i < string.length; i++) {
      let c = string.charCodeAt(i),
          d = string.charCodeAt(i + 1);

      if (c >= 0xd800 && c <= 0xdbff && d >= 0xdc00 && d <= 0xdfff) {
        c = 0x10000 + ((c - 0xd800) << 10) + (d - 0xdc00);
        i += 1;
      }

      if (c >= 0xd800 && c <= 0xdfff) c = 0xfffd;

      if (c <= 0x7f) {
        bytes.push(c);
      }
      else if (c <= 0x7ff) {
        bytes.push(0xc0 | (c >>>  6), 0x80 | c % 0x40);
      }
      else if (c <= 0xffff) {
        bytes.push(0xe0 | (c >>> 12), 0x80 | (c >>>  6) % 0x40, 0x80 | c % 0x40);
      }
      else if (c <= 0x10ffff) {
        bytes.push(0xf0 | (c >>> 18), 0x80 | (c >>> 12) % 0x40, 0x80 | (c >>> 6) % 0x40, 0x80 | c % 0x40);
      }
    }

    return bytes;
  },

  encode(bytes) {
    let codepoints = [],
        offset     = 0,
        n          = bytes.length;

    while (offset < n) {
      let c = bytes[offset];

      let b = [1, 2, 3].map((i) => {
        let _byte = bytes[offset + i];
        return (_byte >>> 6 === 2) ? _byte : NaN;
      });

      if (c <= 0x7f) {
        offset += 1;
      }
      else if (c >= 0xc2 && c <= 0xdf && b[0]) {
        offset += 2;
        c = c % 0x20 <<  6 | b[0] % 0x40;
      }
      else if ((c === 0xe0              && b[0] >= 0xa0 && b[1])
            || (c >=  0xe1 && c <= 0xec && b[0]         && b[1])
            || (c === 0xed              && b[0] <= 0x9f && b[1])
            || (c >=  0xee && c <= 0xef && b[0]         && b[1])) {
        offset += 3;
        c = c % 0x10 << 12 | b[0] % 0x40 <<  6 | b[1] % 0x40;
      }
      else if ((c === 0xf0              && b[0] >= 0x90 && b[1] && b[2])
            || (c >=  0xf1 && c <= 0xf3 && b[0]         && b[1] && b[2])
            || (c === 0xf4              && b[0] <= 0x8f && b[1] && b[2])) {
        offset += 4;
        c = c % 0x08 << 18 | b[0] % 0x40 << 12 | b[1] % 0x40 << 6 | b[2] % 0x40;
      }
      else {
        offset += 1;
        c = 0xfffd;
      }

      if (c <= 0xffff) {
        codepoints.push(c);
      } else {
        c -= 0x10000;
        codepoints.push(0xd800 + (c >>> 10), 0xdc00 + (c % 0x400));
      }
    }

    return String.fromCharCode.apply(String, codepoints);
  }
};
